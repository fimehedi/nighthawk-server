...
class PatreonService {
	...
	async verifyPatronStatus(accessToken) {
		...
		const requiredCampaignId = config.patreon_campaign_id || null;
	...
	async handleOAuthCallback(code) {
		...
		return {
			user: {
				...
				membershipTier: patreonUser.membership_tier,
				isActivePatron: patreonUser.is_active_patron,
				pledgeAmount: patreonUser.pledge_amount_cents / 100, // Convert to dollars
			},
			token: appToken,
		};
	}
	...
	/**
	 * Get all Patreon users (for admin)
	 */
	async getAllPatreonUsers() {
		const users = await prisma.patreonUser.findMany({
			select: {
				id: true,
				email: true,
				full_name: true,
